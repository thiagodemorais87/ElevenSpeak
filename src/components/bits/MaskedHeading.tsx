...
export function MaskedHeading({
  ...
  const mediaContent = fillSlot ? (
    <span className="masked-heading-fill pointer-events-none block h-full w-full overflow-hidden bg-obsidian">
      {fillSlot}
    </span>
  ) : mediaType === 'video' ? (
    <video
  ...
  return (
    <Tag
      ref={rootRef}
      className={`masked-heading relative m-0 w-full p-0 antialiased [text-wrap:balance] ${className}`.trim()}
      style={{
        textAlign: align,
        fontWeight: weight,
